...
(function() {
    ...
    function renderNotification(n) {
        ...
        // Prefer the cover when the API returned one; fall back to the type glyph.
        var iconInner = n.poster
            ? '<img class="kp-noti-cover" src="' + esc(n.poster) + '" alt="" loading="lazy" decoding="async" onerror="this.remove();this.parentNode.classList.remove(\'has-cover\');">'
            : '<i class="' + (n.icon || typeIcon(n.type)) + '"></i>';
        var iconCls = 'kp-noti-icon' + (n.poster ? ' has-cover' : '');

        div.innerHTML =
            '<div class="' + iconCls + '" style="color:' + iconColor + ';">'
          +   iconInner
          + '</div>'
          + '<div class="kp-noti-body">'
          +   '<div class="kp-noti-title">' + esc(n.title || 'Unknown Anime') + '</div>'
          +   '<div class="kp-noti-msg">' + esc(n.message || '') + '</div>'
          +   '<div class="kp-noti-time">' + esc(timeAgo(n.time)) + '</div>'
          + '</div>'
          + '<div class="kp-noti-item-actions">'
          +   '<a class="kp-noti-link" href="' + esc(n.url || watchUrl(n.slug)) + '" title="' + (n.type === 'episode' ? 'Watch now' : 'Open') + '">'
    ...
    function renderEmpty(msg) {
        container.innerHTML =
          ...
          +   '<a class="kp-empty-cta" href="./index.php"><i class="fas fa-fire"></i> Browse trending</a>'
          + '</div>';
    }
